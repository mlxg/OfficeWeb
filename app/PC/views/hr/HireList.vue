  ...
  <div class="records-container" :class="opened ? 'opened': ''">
    <ul class="search_bar records-search">
      ...
      <li class="records-item">
        <span class="item-title">录用人部门</span>
        <div class="item-cont">
                    ...
                    fields: [
                        {
                            ...
                            formatter: function (value, obj) {
                                return '<a class="btn-link" href="#!/hr/HireList/procMyHire/' + obj.id + '/'+obj.flowId+'/0/0">' + value + '</a>'
                            }
                        },
                        {
                            name: 'serialNo',
...
//                                    isShow: function (obj) {
//                                        return obj.status !== this.status.PAUSE
//                                    },
//                                    handler: function (obj, array, index) {
//                                        let vm = this;
//                                        vm.updateType({
//                                            id: obj.id,
//                                            status: vm.status.PAUSE
//                                        }, function () {
//                                            vm.$set('data.data[' + index + '].status', vm.status.PAUSE)
//                                        })
//                                    }
//                                },
//              {
...
//                    status: 3
//                  }, function () {
//                    vm.$set('data.data[' + index + '].status', vm.status.START)
//                  })
//                }
//              },
                                /*{
        ...
        methods: {
            queryEmployee:function (id) {
                let vm = this;
                this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDAPPLICATION, id).then((response)=>{
                    let responseData = JSON.parse(response.data);
                    responseData = responseData.result
                    // console.log(responseData)
                    let pageName = 'newStaffLetter.html',
                    url = '/'+pageName+
                        '?cnName='+responseData.cnName+
                        '&contractCompanyDisplay='+responseData.contractCompanyDisplay+
                        '&employeeNatureId='+responseData.employeeNatureId+
                        '&baseTestSalary='+responseData.baseTestSalary+
                        '&secrecyTestSalary='+responseData.secrecyTestSalary+
                        '&otherSubsidyTestSalary='+responseData.otherSubsidyTestSalary+
                        '&overSubsidyTestSalary='+responseData.overSubsidyTestSalary+
                        '&lunchSubsidyTestSalary='+responseData.lunchSubsidyTestSalary+
                        '&achievementTestSalary='+responseData.achievementTestSalary+
                        '&baseFormalSalary='+responseData.baseFormalSalary+
                        '&secrecyFormalSalary='+responseData.secrecyFormalSalary+
                        '&otherSubsidyFormalSalary='+responseData.otherSubsidyFormalSalary+
                        '&overSubsidyFormalSalary='+responseData.overSubsidyFormalSalary+
                        '&lunchSubsidyFormalSalary='+responseData.lunchSubsidyFormalSalary+
                        '&achievementFormalSalary='+responseData.achievementFormalSalary;
            ...
            _loadData: function (pages) {
                let vm = this;
                this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDAPPLICATIONLIST, pages).then((response)=>{
            ...
            loadData(){
              let vm = this;
              vm.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDAPPLICATIONLIST, vm.search).then((response) => {
                let responseData = response.json().result;
                let page = {};
                page.data = responseData.list;
                page.total = responseData.count;
            ...
            getOfficeList: function () {
                  let vm = this;
                  this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_GETOFFICELISTHIRELIST,{}).then((response) => {
                    let responseData = response.json().result;
            ...
            getStatusList: function () {
                  let vm = this;
                  this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_GETFLOWSTATUSLISTHIRELIST,{}).then((response) => {
                    let responseData = response.json().result;
