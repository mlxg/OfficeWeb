    ...
    export default{
                    ...
                    fields: [
                      {
                        ...
                        formatter: function (value, obj) {
                          return '<a class="btn-link" href="#!/hr/FieldList/AppField/' + obj.id + '/' + obj.flowId + '/0/0">' + value + '</a>'
                        }
                      },
                      {
                        name: 'serialNo',
        ...
        methods: {
            _loadData: function (pages) {
                let vm = this;
                this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDOUTSIDELIST, {bussinessData:pages}).then((response)=>{
            ...
            loadData(){
              let vm = this;
              vm.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDOUTSIDELIST, {bussinessData:vm.search}).then((response) => {
                let responseData = response.json().result;
                let page = {};
                page.data = responseData.list;
                page.total = responseData.count;
            ...
            getOfficeList: function () {
                  let vm = this;
                  this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDOFFICELISTINOUTSIDELIST,{}).then((response) => {
                    let responseData = response.json().result;
            ...
            getStatusList: function () {
                  let vm = this;
                  this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDSTATUSLISTINOUTSIDELIST,{}).then((response) => {
                    let responseData = response.json().result;
