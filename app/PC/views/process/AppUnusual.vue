...
<template>
  ...
  <validator name="validation">
    <Tip-border tip="考勤异常基本信息">
      <div class="g-padding20">
        <div class="form-group clearfix">
          ...
          <div class="col-sm-4">
            <span class="writing">{{datas.officeName}}</span>
          </div>
        </div>
        <div class="form-group clearfix">
          <label class="col-sm-2 control-label">紧急程度：</label>
          <div class="col-sm-4">
            <span class="writing">{{datas.urgencyDegreeDispay}}</span>
          </div>
        </div>
      </div>
    </Tip-border>
    <Tip-border tip="考勤异常详细信息">
      <div class="g-padding20">
        <div class="form-group clearfix" v-for="item in datas.workTimeList">
          <label class="col-sm-2 control-label">考勤异常日期：</label>
          <div class="col-sm-2">
            <span class="writing">{{item.exceptionTime}}</span>
          </div>
          <div class="col-sm-4">
            <span class="writing">{{item.timeTypeDisplay}}</span>
          </div>
        </div>
        <div class="form-group clearfix">
          <label class="col-sm-2 control-label">异常考勤次数：</label>
          <div class="col-sm-4">
            <span class="writing">{{objlen}}</span>
          </div>
          <label class="col-sm-3 control-label">本月异常考勤次数：</label>
          <div class="col-sm-2">
            <span class="writing">{{datas.exceptionMonthTimes}}</span>
          </div>
        </div>
        <div class="form-group g-marginB20 clearfix">
          <label class="col-sm-2 control-label">申请理由：</label>
          <div class="col-sm-4">
            <span class="writing">{{datas.applyReasons}}</span>
          </div>
        </div>
    ...
    <div class="form-group g-tc g-marginT20" v-if="this.status == '1' || this.status == '6'">
      <button type="button" class="btn btn-primary g-marginR30" @click="deleteTask" v-if='types == "application"'>删除
      </button>
      <button type="button" class="btn btn-primary g-marginR30" @click="withdraw">撤回</button>
      <button v-if="this.types=='application'" type="button" class="btn btn-primary g-marginR30" @click="reminders">催办
      </button>
  ...
  export default{
    ...
    methods: {
      ...
      loadData: function () {
        ...
        this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_GETWORKTIMEEXCEPTION, id).then(function (response) {
          let Data = response.json().result;
          that.datas = Data;

          //获取异常次数
          that.datas.workTimeList = Data.workTimeList;
          that.objlen = 0;
          for (var i in that.datas.workTimeList) {
            that.objlen++;
          }
          return that.objlen;
        })
      ...
      deleteTask: function () {
        ...
        vm.confirm = {
          ...
          callback: function () {
            ...
            vm.$http.post(content.STAROFFICE_ACT + vm.apiUrl, {
      ...
      withdraw: function () {
        ...
        this.$http.post(content.STAROFFICE_ACT + this.apiUrl, {
      ...
      reminders: function () {
        ...
        if (!this.types)return false;
        this.userId = sessionStorage.getItem("loginName");
        this.$http.post(content.STAROFFICE_ACT + interfaces.INTERFACE_REMINDERTASK, {
          humanTaskId: this.humanTaskId,
          userId: this.userId
        }).then(function (response) {
          let Data = response.json().status;
