    ...
    <Tip-border tip="考勤异常基本信息">
      <div class="g-padding20">
        ...
        <div class="form-group clearfix">
          ...
          <div class="col-sm-4" >
            <span class="writing">{{urgencyDegreeDispay}}</span>
          </div>
        </div>
      </div>
    </Tip-border>
...
<script type="text/ecmascript-6">
  ...
  asyncData: function (resolve, reject) {
    this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_GETWORKTIMEEXCEPTION,{bussinessData: this.businessKey}).then(function (response) {
      var Data = response.json().result;
      resolve({
        'datas.id' : Data.id,
        'datas.serialNo': Data.serialNo,
        'datas.title': Data.title,
        'datas.urgencyDegreeId': Data.urgencyDegreeId,
        'datas.applyReasons': Data.applyReasons,
        'officeName': Data.officeName,
        'urgencyDegreeDispay': Data.urgencyDegreeDispay,
        'datas.workTimeList': Data.workTimeList,
        'datas.exceptionMonthTimes': Data.exceptionMonthTimes,
      })
      this.datas.workTimeList = Data.workTimeList;
      this.objlen = 0;
      for (var i in this.datas.workTimeList) {
        this.objlen++;
      }
      return this.objlen;
