...
<template>
  ...
  <Tip-border tip="加班申请基本信息">
    <div class="g-padding20">
      ...
      <div class="form-group clearfix">
        ...
        <div class="col-sm-4">
          <span class="writing">{{datas.overtimeType}}</span>
        </div>
      </div>
      <div class="form-group clearfix">
        <label class="col-sm-2 control-label">补贴类型：</label>
        <div class="col-sm-4">
          <span class="writing">{{datas.subsidyType}}</span>
        </div>
      </div>
    </div>
  </Tip-border>
  ...
  <Tip-border tip="计薪/计时信息" v-if="(taskform == 'holiday_employee_kqgly' ||taskform =='holiday_head_kqgly') &&  datas.overtimeSubsidyType =='4'">
    <div class="g-padding20">
      <div class="form-group clearfix">
        <label class="col-sm-2 control-label">计薪时数：</label>
        <div class="col-sm-4">
          <span class="writing">{{datas.paymentHours}}</span>
        </div>
        <label class="col-sm-2 control-label">加班时数：</label>
        <div class="col-sm-4">
          <span class="writing">{{datas.overtimeHours}}</span>
        </div>
      </div>
    </div>
  </Tip-border>
    ...
    export default{
        ...
        methods : {
            ...
            loadData: function () {
                ...
                this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDOVERTIME,id).then(function (response) {
