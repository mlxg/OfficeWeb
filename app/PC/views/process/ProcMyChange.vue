<!-- 人事管理 --- 人事管理 --- 人事异动 -->
<template>
    <h2 class="g-tc u-fs20 g-marginB20">{{post.bussinessData.title}}</h2>
    <validator name="validation">
        <!--<form class="clearfix" novalidate>-->
        <Tip-border tip="基本信息">
            <div class="g-padding20">
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label">标题：</label>
                    <div class="col-sm-4">
                        <span class="writing">{{post.bussinessData.title}}</span>
                    </div>
                    <label class="col-sm-2 control-label">编号：</label>
                    <div class="col-sm-4">
                        <span class="writing">{{post.bussinessData.serialNo}}</span>
                    </div>
                </div>
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label">紧急程度：</label>
                    <div class="col-sm-4" >
                        <span class="writing">{{urgencyDegreeDisplay}}</span>
                    </div>
                </div>
            </div>
        </Tip-border>
        <Tip-border tip="异动情况">
            <div class="g-padding20">
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label">姓名：</label>
                    <div class="col-sm-4">
                        <span class="writing">{{cnName}}</span>
                    </div>
                    <label class="col-sm-2 control-label">员工编号：</label>
                    <div class="col-sm-4">
                        <span class="writing">{{employeeNo}}</span>
                    </div>
                </div>
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label">变动性质：</label>
                    <div class="col-sm-4" v-validate-class>
                        <span class="writing">{{moveNatureDisplay}}</span>
                    </div>
                    <label class="col-sm-2 control-label">入职时间：</label>
                    <div class="col-sm-4">
                        <span class="writing">{{post.bussinessData.entrantTime}}</span>
                    </div>
                </div>
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label field-require">是否部门负责人：</label>
                    <div class="col-sm-4" v-if="post.bussinessData.afterIsResponsible == 1">
                        <span class="writing">是</span>
                    </div>
                    <div class="col-sm-4" v-else="">
                        <span class="writing">否</span>
                    </div>
                    <label class="col-sm-2 control-label">建议生效日期：</label>
                    <div class="col-sm-4" v-validate-class >
                        <div v-if="taskformKey == 'move_hr'">
                            <input type="text" class="form-control input-calendar" v-model="post.bussinessData.suggestEffectDate"
                                   v-validate:entrant-time="{required: true}"
                                   @click.stop="calendar.show = true" readonly>
                            <span class="fc-red" v-show="($validation.entrantTime.touched || validating) && $validation.entrantTime.required">必填项</span>
                            <Public-Calendar
                                    :value.sync="post.bussinessData.suggestEffectDate"
                                    :show.sync="calendar.show"
                                    :x="calendar.x"
                                    :y="calendar.y"
                                    :sep.sync="calendar.sep"></Public-Calendar>
                        </div>
                        <div v-else>
                            <span class="writing">{{post.bussinessData.suggestEffectDate}}</span>
                        </div>
                    </div>
                </div>
                <div class="form-group g-marginB20 clearfix" v-validate-class>
                    <label class="col-sm-2 control-label field-require">变动情况说明：</label>
                    <div class="col-sm-4" v-validate-class>
                        <span class="writing">{{post.bussinessData.moveCaseExplain}}</span>
                    </div>
                </div>
                <div class="form-group g-marginB20 clearfix">
                    <label class="col-sm-2 control-label">变动具体情况：</label>
                    <table width="83%" class="table table-striped table-bordered w83 g-fr">
                        <thead>
                        <th>名称</th>
                        <th>异动前</th>
                        <th><div class="col-sm-12">异动后</div></th>
                        </thead>
                        <tbody>
                        <tr>
                            <th><span class="writing">办公地点</span></th>
                            <td><span class="writing">{{beforeWorkplaceDisplay}}</span></td>
                            <td><div class="col-sm-12"><span class="writing">{{afterWorkplaceDisplay}}</span></div></td>
                        </tr>
                        <tr>
                            <th><span class="writing">部门</span></th>
                            <td><span class="writing">{{beforeOfficeName}}</span></td>
                            <td><div class="col-sm-12"><span class="writing">{{afterOfficeName}}</span></div></td>
                        </tr>
                        <tr>
                            <th><span class="writing">职位</span></th>
                            <td><span class="writing">{{beforePositionsName}}</span></td>
                            <td><div class="col-sm-12"><span class="writing">{{afterPositionsName}}</span></div></td>
                        </tr>
                        <tr>
                            <th><span class="writing">职级</span></th>
                            <td><span class="writing">{{beforePositionsLevelDisplay}}</span></td>
                            <td><div class="col-sm-12"><span class="writing">{{afterPositionsLevelDisplay}}</span></div></td>
                        </tr>
                        <tr>
                            <th><span class="writing">月薪</span></th>
                            <td><span class="writing">{{post.bussinessData.beforeMonthSalary}}</span></td>
                            <td v-validate-class>
                                <div v-if="taskformKey == 'move_hr'" class="col-sm-4">
                                    <input type="text" class="form-control" v-model="post.bussinessData.afterMonthSalary" maxlength="9"
                                           v-validate:trym="{required: { rule: true }, pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}">
                                    <span class="fc-red" v-show="($validation.trym.touched || validating) && $validation.trym.required">必填项</span>
                                    <span class="fc-red" v-show="post.bussinessData.afterMonthSalary && $validation.trym.touched && $validation.trym.pattern">金额格式错误</span>
                                </div>
                                <div v-else class="col-sm-12">
                                    <span class="writing">{{post.bussinessData.afterMonthSalary}}</span>
                                </div>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                <div class="form-group clearfix">
                    <label class="col-sm-2 control-label">附件下载：</label>
                    <div class="col-sm-4">
                        <a class="upload-down btn-link" v-for="item in fileList" v-if="item" :href="item.fastPath" :download="item.filename">{{item.filename}}</a>
                    </div>
                </div>
            </div>
        </Tip-border>
        <Tip-border tip="人力资源部复核" v-if="taskformKey == 'move_hr'">
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">基本工资(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeBaseFormalSalary" readonly >
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterBaseFormalSalary" maxlength="9"
                               v-validate:afterbaseformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.afterbaseformalsalary.touched || validating) && $validation.afterbaseformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterBaseFormalSalary && $validation.afterbaseformalsalary.touched && $validation.afterbaseformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">保密工资(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeSecrecyFormalSalary" readonly>
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterSecrecyFormalSalary" maxlength="9"
                               v-validate:aftersecrecyformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.aftersecrecyformalsalary.touched || validating) && $validation.aftersecrecyformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterSecrecyFormalSalary && $validation.aftersecrecyformalsalary.touched && $validation.aftersecrecyformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">其他补贴(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeOverSubsidyFormalSalary" readonly>
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterOverSubsidyFormalSalary" maxlength="9"
                               v-validate:afteroversubsidyformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.afteroversubsidyformalsalary.touched || validating) && $validation.afteroversubsidyformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterOverSubsidyFormalSalary && $validation.afteroversubsidyformalsalary.touched && $validation.afteroversubsidyformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">加班补贴(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeOtherSubsidyFormalSalary" readonly>
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterOtherSubsidyFormalSalary" maxlength="9"
                               v-validate:afterothersubsidyformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.afterothersubsidyformalsalary.touched || validating) && $validation.afterothersubsidyformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterOtherSubsidyFormalSalary && $validation.afterothersubsidyformalsalary.touched && $validation.afterothersubsidyformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">午餐补助(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeLunchSubsidyFormalSalary" readonly>
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterLunchSubsidyFormalSalary" maxlength="9"
                               v-validate:afterlunchsubsidyformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.afterlunchsubsidyformalsalary.touched || validating) && $validation.afterlunchsubsidyformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterLunchSubsidyFormalSalary && $validation.afterlunchsubsidyformalsalary.touched && $validation.afterlunchsubsidyformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>
                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">绩效考核(异动前)</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.beforeAchievementFormalSalary"readonly>
                    </div>
                    <label class="col-sm-2 control-label g-tr">（异动后）</label>
                    <div class="col-sm-4" v-validate-class>
                        <input type="text" class="form-control" v-model="post.bussinessData.afterAchievementFormalSalary" maxlength="9"
                               v-validate:afterachievementformalsalary="{required: { rule: true },pattern: '/^(([1-9]{1}\\d*)|(0))(\\.(\\d){0,2})?$/'}" >
                        <span class="fc-red" v-show="($validation.afterachievementformalsalary.touched || validating) && $validation.afterachievementformalsalary.required">必填项</span>
                        <span class="fc-red" v-show="post.bussinessData.afterAchievementFormalSalary && $validation.afterachievementformalsalary.touched && $validation.afterachievementformalsalary.pattern">金额格式错误</span>
                    </div>
                </div>

                <div class="g-marginT20 g-marginB20 clearfix">
                    <label class="col-sm-2 control-label g-paddingR0">合计：</label>
                    <div class="col-sm-4">
                      <span class="writing totalRed">{{hrTotals}}</span>
                    </div>
                    <label class="col-sm-2 control-label g-paddingR0">合计：</label>
                    <div class="col-sm-4">
                      <span class="writing totalRed">{{hrFormalTotals}}</span>
                    </div>
                </div>
        </Tip-border>
        <Tip-border tip="人力资源部复核" v-if="taskformKey == 'move_zjl' || taskformKey == 'move_zc' || taskformKey == 'move_mover_confirm'">
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">基本工资(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeBaseFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterBaseFormalSalary" readonly>
                </div>
            </div>
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">保密工资(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeSecrecyFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterSecrecyFormalSalary" readonly>
                </div>
            </div>
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">其他补贴(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeOverSubsidyFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterOverSubsidyFormalSalary" readonly>
                </div>
            </div>
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">加班补贴(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeOtherSubsidyFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterOtherSubsidyFormalSalary" readonly>
                </div>
            </div>
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">午餐补助(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeLunchSubsidyFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterLunchSubsidyFormalSalary" readonly>
                </div>
            </div>
            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">绩效考核(异动前)</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.beforeAchievementFormalSalary" readonly>
                </div>
                <label class="col-sm-2 control-label g-tr">（异动后）</label>
                <div class="col-sm-4" v-validate-class>
                    <input type="text" class="form-control" v-model="post.bussinessData.afterAchievementFormalSalary" readonly>
                </div>
            </div>

            <div class="g-marginT20 g-marginB20 clearfix">
                <label class="col-sm-2 control-label g-paddingR0">合计：</label>
                <div class="col-sm-4">
                    <span class="writing totalRed">{{hrTotals}}</span>
                </div>
                <label class="col-sm-2 control-label g-paddingR0">合计：</label>
                <div class="col-sm-4">
                    <span class="writing totalRed">{{hrFormalTotals}}</span>
                </div>
            </div>
        </Tip-border>
        <Process-Diagram v-if="this.taskId != 0" :process-instance-id="taskId"></Process-Diagram>
        <Tip-border v-if="this.humanTaskId != 0" tip="审批意见">
            <Approve-List :human-task-id="humanTaskId"></Approve-List>
        </Tip-border>
        <Process-Approve v-if="humanTaskId" :comments="comments" :comment="comment" :show.sync="isShow" :id="humanTaskId"></Process-Approve>
        <div class="form-group g-tc g-marginT20">
            <button type="button" class="btn btn-primary g-marginR30" @click="approveProcess($event,true)">同意</button>
            <button type="button" v-if="taskformKey != 'move_mover_confirm'" class="btn btn-default g-marginR30" @click="approveProcess($event,false)">驳回</button>
            <button type="button" class="btn btn-default" @click="cancelProcess">取消</button>
        </div>
    </validator>


    <Public-Modal :options="options" :confirm="confirm" :cancel="cancel" :close="close"></Public-Modal>
    <Look-Screen :lookscreen="lookscreen"></Look-Screen>
</template>

<script type="text/ecmascript-6">
    import TipBorder from '../../components/public/TipBorder.vue'
    import PublicCalendar from '../../components/public/PublicCalendar'
    import PublicModal from '../../components/public/PublicModal'
    import ProcessApprove from '../../components/others/ProcessApprove';
    import LookScreen from '../../components/public/LockScreen.vue'
    import ProcessDiagram from '../../components/others/ProcessDiagram';
    import ApproveList from '../../components/others/ApproveList';
    import { getData, changeCurRoute, initCurRoute } from '../../actions/headNavbarAction'
    import { getNavBar, getUrl, getCurRoute, getIgnoreRoute, getSingleRoute } from '../../getters/headNavbarGetter'
    import * as content from '../../constants/contentTypes';
    import * as interfaces from '../../constants/interfaceTypes';
    export default{
        vuex: {
            actions: {
                changeCurRoute
            },
            getters: {
                navData: getNavBar,
                url: getUrl,
                curRoute: getCurRoute,
                ignoreRoute: getIgnoreRoute,
                singleRoute: getSingleRoute
            }
        },
        props: ['taskId', 'businessKey', 'humanTaskId','taskformKey'],
        components: {
            TipBorder,
            ProcessApprove,
            PublicCalendar,
            PublicModal,
            Text,
            LookScreen,
            ProcessDiagram,
            ApproveList
        },
        computed:{
            hrTotals: function(){
                let vm = this;
                let beforeBaseFormalSalary = parseInt(vm.post.bussinessData.beforeBaseFormalSalary) || 0;
                let beforeSecrecyFormalSalary = parseInt(vm.post.bussinessData.beforeSecrecyFormalSalary) || 0;
                let beforeOtherSubsidyFormalSalary = parseInt(vm.post.bussinessData.beforeOtherSubsidyFormalSalary) || 0;
                let beforeOverSubsidyFormalSalary = parseInt(vm.post.bussinessData.beforeOverSubsidyFormalSalary) || 0;
                let beforeAchievementFormalSalary = parseInt(vm.post.bussinessData.beforeAchievementFormalSalary) || 0;
                return beforeBaseFormalSalary + beforeSecrecyFormalSalary + beforeOtherSubsidyFormalSalary + beforeOverSubsidyFormalSalary + beforeAchievementFormalSalary;
            },
            hrFormalTotals: function(){
                let vm = this;
                let afterBaseFormalSalary = parseInt(vm.post.bussinessData.afterBaseFormalSalary) || 0;
                let afterSecrecyFormalSalary = parseInt(vm.post.bussinessData.afterSecrecyFormalSalary) || 0;
                let afterOtherSubsidyFormalSalary = parseInt(vm.post.bussinessData.afterOtherSubsidyFormalSalary) || 0;
                let afterOverSubsidyFormalSalary = parseInt(vm.post.bussinessData.afterOverSubsidyFormalSalary) || 0;
                let afterAchievementFormalSalary = parseInt(vm.post.bussinessData.afterAchievementFormalSalary) || 0;
                return afterBaseFormalSalary + afterSecrecyFormalSalary + afterOtherSubsidyFormalSalary + afterOverSubsidyFormalSalary  + afterAchievementFormalSalary;
            }
        },
        data: function(){
            return {
                comment:'',
                comments:false,
                isShow: false,
                bussinessData:{
                    id:'',
                },
                cnName:'',
                employeeNo:'',
                post: {
                    bussinessData:{
                        id: '',                                             //主键
                        title:'',                                            //标题
                        serialNo: '',                                       //招聘编号
                        formKey:'',
                        employeeBaseId:'',                                 //员工基础信息ID
                        urgencyDegreeId:'',                                 //紧急程度，数据字典值
                        moveNatureId:'',                                 //	异动性质，数据字典值
                        suggestEffectDate:'',                                 //建议生效日期
                        moveCaseExplain:'',                                 //变动情况说明
                        beforeIsResponsible:'',                                 //异动前是否是部门负责人
                        afterIsResponsible:'0',                                 //异动后是否是部门负责人
                        beforeWorkplaceId:'',                                 //异动前办公地点，数据字典值
                        afterWorkplaceId:'',                                 //异动后办公地点，数据字典值
                        beforeOfficeId:'',                                 //异动前部门ID
                        afterOfficeId:'',                                 //异动后部门ID
                        beforePositionsId:'',                                 //异动前职位ID
                        afterPositionsId:'',                                 //异动后职位ID
                        beforePositionsLevelId:'',                          //异动前职级，数据字典值
                        afterPositionsLevelId:'',                           //异动后职级，数据字典值
                        beforeMonthSalary:'',                                 //异动前月薪
                        afterMonthSalary:'',                                 //异动后月薪

                        beforeBaseFormalSalary:'',
                        afterBaseFormalSalary:'',
                        beforeSecrecyFormalSalary:'',
                        afterSecrecyFormalSalary:'',
                        beforeOtherSubsidyFormalSalary:'',
                        afterOtherSubsidyFormalSalary:'',
                        beforeOverSubsidyFormalSalary:'',
                        afterOverSubsidyFormalSalary:'',
                        beforeLunchSubsidyFormalSalary:'',
                        afterLunchSubsidyFormalSalary:'',
                        beforeAchievementFormalSalary:'',
                        afterAchievementFormalSalary:'',
                    },
                },
                calendar: {
                    show: false,
                    x: 13,
                    y: 40,
                    sep: '-',
                },
                urgencyDegreeDisplay:'',
                moveNatureDisplay:'',
                beforeWorkplaceDisplay:'',
                afterWorkplaceDisplay:'',
                beforeOfficeName:'',
                afterOfficeName:'',
                beforePositionsName:'',
                afterPositionsName:'',
                beforePositionsLevelDisplay:'',
                afterPositionsLevelDisplay:'',
                fileList:[],

                validating: false,

                options: undefined,
                confirm: undefined,
                cancel: undefined,
                close: undefined,
                lookscreen:{
                    isShow:false,
                },
            }
        },


        asyncData: function (resolve, reject) {
            let that = this;
            let id = {
                bussinessData:{
                    id:that.businessKey
                }
            }
            this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDMOVE,id).then(function (response) {
                var Data = response.json().result;
                resolve({
                    'post.bussinessData.id' : Data.id,                                                    //录用申请信息主键
                    'post.bussinessData.serialNo' : Data.serialNo,
                    'post.bussinessData.title' : Data.title,
                    'post.bussinessData.employeeBaseId' : Data.employeeBaseId,
                    'post.bussinessData.entrantTime' : Data.entrantTime,
                    'post.bussinessData.suggestEffectDate' : Data.suggestEffectDate,
                    'post.bussinessData.moveCaseExplain' : Data.moveCaseExplain,
                    'post.bussinessData.urgencyDegreeId' : Data.urgencyDegreeId,
                    'post.bussinessData.moveNatureId' : Data.moveNatureId,
                    'post.bussinessData.beforeIsResponsible' : Data.beforeIsResponsible,
                    'post.bussinessData.afterIsResponsible' : Data.afterIsResponsible,
                    'post.bussinessData.beforeWorkplaceId' : Data.beforeWorkplaceId,
                    'post.bussinessData.afterWorkplaceId' : Data.afterWorkplaceId,
                    'post.bussinessData.beforeOfficeId' : Data.beforeOfficeId,
                    'post.bussinessData.afterOfficeId' : Data.afterOfficeId,
                    'post.bussinessData.beforePositionsId' : Data.beforePositionsId,
                    'post.bussinessData.afterPositionsId' : Data.afterPositionsId,
                    'post.bussinessData.beforePositionsLevelId' : Data.beforePositionsLevelId,
                    'post.bussinessData.afterPositionsLevelId' : Data.afterPositionsLevelId,
                    'post.bussinessData.beforeMonthSalary' : Data.beforeMonthSalary,
                    'post.bussinessData.afterMonthSalary' : Data.afterMonthSalary,

                    'post.bussinessData.beforeBaseFormalSalary' : Data.beforeBaseFormalSalary,
                    'post.bussinessData.afterBaseFormalSalary' : Data.afterBaseFormalSalary,
                    'post.bussinessData.beforeSecrecyFormalSalary' : Data.beforeSecrecyFormalSalary,
                    'post.bussinessData.afterSecrecyFormalSalary' : Data.afterSecrecyFormalSalary,
                    'post.bussinessData.beforeOtherSubsidyFormalSalary' : Data.beforeOtherSubsidyFormalSalary,
                    'post.bussinessData.afterOtherSubsidyFormalSalary' : Data.afterOtherSubsidyFormalSalary,
                    'post.bussinessData.beforeOverSubsidyFormalSalary' : Data.beforeOverSubsidyFormalSalary,
                    'post.bussinessData.afterOverSubsidyFormalSalary' : Data.afterOverSubsidyFormalSalary,
                    'post.bussinessData.beforeLunchSubsidyFormalSalary' : Data.beforeLunchSubsidyFormalSalary,
                    'post.bussinessData.afterLunchSubsidyFormalSalary' : Data.afterLunchSubsidyFormalSalary,
                    'post.bussinessData.beforeAchievementFormalSalary' : Data.beforeAchievementFormalSalary,
                    'post.bussinessData.afterAchievementFormalSalary' : Data.afterAchievementFormalSalary,

                    'employeeNo' : Data.employeeNo,
                    'cnName' : Data.cnName,
                    'urgencyDegreeDisplay' : Data.urgencyDegreeDisplay,
                    'moveNatureDisplay' : Data.moveNatureDisplay,
                    'beforeWorkplaceDisplay' : Data.beforeWorkplaceDisplay,
                    'afterWorkplaceDisplay' : Data.afterWorkplaceDisplay,
                    'beforeOfficeName' : Data.beforeOfficeName,
                    'afterOfficeName' : Data.afterOfficeName,
                    'beforePositionsName' : Data.beforePositionsName,
                    'afterPositionsName' : Data.afterPositionsName,
                    'beforePositionsLevelDisplay' : Data.beforePositionsLevelDisplay,
                    'afterPositionsLevelDisplay' : Data.afterPositionsLevelDisplay,
                })
            })
            this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_FINDATTACHMENTLIST, {resourceId: this.businessKey}).then((response) => {
                resolve({
                    'fileList': response.json().result
                })
            })
        },
        created: function(){
            let hash = window.location.hash.slice(1).match(/\/\w+/g).reverse();
            console.log(hash)
            this.humanTaskId = hash[1].match(/\w+/)[0];
            this.taskId = hash[2].match(/\w+/)[0];
        },
        methods : {
            approveProcess: function (e,comments) {
                let vm = this;
                vm.comments = comments;
                if(comments){
                    vm.comment = '同意'
                }else{
                    vm.comment = '不同意'
                }
                if(vm.taskformKey == "move_hr" ){
                    this.$validate(true, function () {
                        if (vm.$validation.invalid) {
                            e.preventDefault()
                        }else{
                            vm.sub();
                        }
                    })
                }else{
                    vm.sub();
                }
            },
            sub: function(){
                let vm = this;
                //数组转换成字符串
                vm.isShow = true;
                vm.post.bussinessData.formKey = vm.taskformKey
                this.$http.post(content.CONTENT_ROOT + interfaces.INTERFACE_SAVEMOVE, this.post).then((response) => {
                    let status = response.json().status;
                    if(status == 0){
                        vm.isShow = true;
                    }else{
                    }

                });
            },
            cancelProcess: function () {
                this.$router.go('/process/todo')
            },


        }
    }
</script>

<style lang="less">
    table.w83{
        width:83%
    }
    table.mask-table td{
        text-align: left;
    }
    .newTab{width:100px}
    .v-red{
        background: #ffe5e6 url(data:image/png;base64,) no-repeat 2px center/14px 14px;
        border: 1px solid #ff7a7a;
        border-radius: 4px;
        height: 24px;
        color: #ff3333;
        font-size: 12px;
        display: inline-block;
        margin-top: -12px;
        position: absolute;
        top: 50%;
        right: 19px;
        padding: 2px 6px 2px 20px;
        display: none;;
    }
    .invalid p{
        display: block;
    }
    .untouched p{
        display: none;
    }

    .mask1{
        position: fixed;
        z-index: 999;
        top: 0;
        right: 0;
        bottom: 0;
        left: 0;
        display: block;
        text-align: center;
        /* inorder to adapt the color #ccc */
        background: rgba(0, 0, 0, 0.20);
    .mask-align{
        display: inline-block;
        height: 100%;
        width: 1px;
        line-height: 100%;
        vertical-align: middle;
    }
    .notice-panel{
        display: inline-block;
        width: 1045px;
        height: auto;
        background-color: #fff;
        position: relative;
        vertical-align: middle;
        margin-top: 60px;
    }
    .notice-title{
        background-color: #278eed;
    h1{
        margin: 0;
        padding-left: 10px;
        text-align: left;
        font-size: 16px;
        height: 40px;
        line-height: 40px;
        color: white;
    }
    }
    .exit{
        width: 20px;
        height: 20px;
        background-image: url(data:image/png;base64,);
        position: absolute;
        right: 10px;
        top: 10px;
    &:hover{
         cursor: pointer;
     }
    }
    .notice-content{
    i{
        width: 30px;
        height: 30px;
        display: inline-block;
        margin-top: 55px;
    }
    .success{
        background-image: url(data:image/png;base64,);
        background-repeat: no-repeat;
        background-position: center;
    }
    .warning{
        background-image: url(data:image/png;base64,);
        background-repeat: no-repeat;
        background-position: center;
    }
    width: 100%;
    height: auto;
    display: flex;
    justify-content: space-between;
    padding: 20px;
    .content{
        vertical-align: middle;
        font-size: 16px;
        margin: 10px;
        display: flex;
        align-items: center;
    }
    }
    .notice-select{
        position: absolute;
        width: 100%;
        bottom: 20px;
    button{
        outline: none;
        border: 1px solid #ddd;
        background-color: white;
        color: #333;
        border-radius: 3px;
        height: 34px;
        line-height: 34px;
        vertical-align: middle;
        padding: 0 24px;
        margin: 0 20px;
    &:hover{
         border: 1px solid #2380d5;
         background-color: #278eed;
         color: white;
         cursor: pointer;
     }
    }
    }
    }
</style>
