...
<template>
    <table :class="config.className || 'table table-striped table-hover'">
        ...
        <tr v-if="!curData">
            <td colspan="{{config.fields.length}}" class="text-center" :style="'height:'+ height +'px; line-height: '+ height +'px'">
                loading
            </td>
        </tr>
...
<script>
    ...
    export default {
        ...
        directives: {
          comp: function (curData) {
            if(!curData) return;
            this.vm.height = this.el.clientHeight;
          },
