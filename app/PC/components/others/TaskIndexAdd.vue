<style>
    .add_pop_staff {position: fixed; padding: 20px; top:50%; left: 50%; width: 300px;height: 250px;
        margin: -150px 0 0 -125px;z-index: 100;background-color: white;border-radius: 5px;transition: all .5s ease;}
    ...
    section.invalid .fc-red{top: 6px; right: 5px; margin: 0;}
</style>
<template>
    <Tool-box></Tool-box>
    <div class="add_pop_staff" transition="add_pop_staff" transition-mode="out-in">
        ...
        <section class="invalid">
            <input class="input" type="text" placeholder="输入项目名" v-on:focus="invalid=false" @keyup.enter="addProj" v-model="title">
            <span class="fc-red" v-show="invalid">必填项</span>
            <Btm-cnt
    ...
    export default{
        ...
        methods:{
            addProj(){
                ...
                if (!title) this.invalid = true;
                else this.taskIndexAddProjFun(params);
            }
        },
        ready(){
